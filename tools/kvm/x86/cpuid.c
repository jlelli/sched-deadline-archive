...
static void filter_cpuid(struct kvm_cpuid2 *kvm_cpuid)
{
	...
	for (i = 0; i < kvm_cpuid->nent; i++) {
		...
		case 10: { /* Architectural Performance Monitoring */
			...
			/*
			 * If we're on the AMD system where KVM itself supports
			 * PMU virtualization with help of architectural events,
			 * we still get version 0 here, so guess if we can
			 * continue by looking into additional fields.
			 */
			if (entry->eax) {
				eax.full = entry->eax;
